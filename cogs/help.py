...
class Helpful(commands.Cog):
    ...
    async def help(self, ctx):
        ...
        embed.add_field(name=".opinion / .mp / .preference", value="Returns the bot's current ping", inline=False)
        ...
        embed.add_field(name=".clear / .sweep / .cls / .clean", value="Deletes the passed amount of messages. Default is 10", inline=False)
        ...
        embed.add_field(name=".load", value="Loads an command so it can be used", inline=False)
        embed.add_field(name=".unload", value="Unloads an command so it can no longer be used", inline=False)
